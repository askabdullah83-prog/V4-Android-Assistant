...
class V4VoiceService : Service(), TextToSpeech.OnInitListener {
    ...
    private val appAliases = mapOf(
        "whatsapp" to listOf("হোয়াটসঅ্যাপ", "হোয়াটসঅ্যাপ", "whats app"),
        "facebook" to listOf("ফেসবুক"),
        "messenger" to listOf("মেসেঞ্জার"),
        "instagram" to listOf("ইনস্টাগ্রাম", "ইনস্টা"),
        "youtube" to listOf("ইউটিউব"),
        "chrome" to listOf("ক্রোম"),
        "tiktok" to listOf("টিকটক"),
        "gmail" to listOf("জিমেইল"),
        "maps" to listOf("ম্যাপ", "গুগল ম্যাপ"),
        "camera" to listOf("ক্যামেরা"),
        "gallery" to listOf("গ্যালারি", "ফটো"),
        "settings" to listOf("সেটিংস"),
        "play store" to listOf("প্লে স্টোর"),
        "free fire" to listOf("ফ্রি ফায়ার", "ফ্রি ফায়ার"),
    ...
    private fun notification(): Notification = NotificationCompat.Builder(this, CHANNEL_ID)
        ...
        .setContentText("Hey JARVIS • সব অ্যাপ খোলা/বন্ধ")
    ...
    private fun ensureAppsLoaded() {
        if (System.currentTimeMillis() - appsLoadedAt > 5 * 60 * 1000L || installedApps.isEmpty()) loadInstalledApps()
    }
    ...
    private fun findAppBySpokenName(spoken: String): AppEntry? {
        ...
        for ((en, bnList) in appAliases) {
            if ((listOf(en) + bnList).any { query.contains(it) || it.contains(query) }) {
                installedApps.firstOrNull { app -> app.labelLower.contains(en) || bnList.any { app.labelLower.contains(it) } }?.let { return it }
            }
        }
        installedApps.firstOrNull { it.labelLower.contains(query) }?.let { return it }
        val words = query.split(" ").filter { it.length > 1 }
        if (words.isNotEmpty()) {
            val scored = installedApps.map { app -> app to words.count { w -> app.labelLower.contains(w) } }.filter { it.second > 0 }.maxByOrNull { it.second }
            if (scored != null) return scored.first
        }
        return null
    }
    ...
    private fun startRecognition() {
        ...
        if (!SpeechRecognizer.isRecognitionAvailable(this)) {
            handler.postDelayed({ startRecognition() }, 2000); return
        }
        recognizer?.destroy()
        ...
        recognizer?.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onResults(results: Bundle?) {
                ...
                if (text.isBlank()) handler.postDelayed({ startRecognition() }, 350) else handleVoice(text)
            }
        })
    ...
    private fun handleVoice(text: String) {
        ...
        if (command.isBlank()) { handler.postDelayed({ startRecognition() }, 350); return }
    ...
    private fun executeCommand(original: String) {
        ...
        val openMatch = Regex("(?:খোলো|খোল|খুলে দাও|চালু করো|চালু কর|open|launch|start)\\s+(.+)|(.+?)\\s+(?:খোলো|খোল|খুলে দাও|চালু করো|চালু কর|open)").find(command)
        if (openMatch != null) {
            val name = (openMatch.groupValues.getOrNull(1) ?: openMatch.groupValues.getOrNull(2) ?: "").replace(Regex("^(the|একটা|একটি)\\s+"), "").trim()
            if (name.length >= 2 && openAppByName(name)) {
                handler.postDelayed({ startRecognition() }, 1400); return
            }
        }
        if ((command.length in 2..40) && (command.split(" ").size <= 3) && openAppByName(command)) {
            handler.postDelayed({ startRecognition() }, 1400); return
        }

        when {
            command.contains("home") || command.contains("হোম") || command.contains("হোমে যাও") || command.contains("হোম স্ক্রিন") -> {
                speakSiri("হোম স্ক্রিনে নিয়ে যাচ্ছি"); JarvisAccessibilityService.goHome()
            }
            command.contains("বন্ধ করো") || command.contains("বন্ধ কর") || command.contains("বন্ধ করে দাও") ||
                command.contains("close app") || command.contains("close") || command.contains("exit app") || command.contains("অ্যাপ বন্ধ") -> {
            ...
            command.contains("পেছনে") || command.contains("back") || command.contains("ফিরে যাও") -> {
                if (JarvisAccessibilityService.pressBack()) speakSiri("পেছনে যাচ্ছি") else speakSiri("Accessibility অনুমতি লাগবে")
            }
            command.contains("volume up") || command.contains("ভলিউম বাড়াও") || command.contains("ভলিউম বাড়াও") || command.contains("শব্দ বাড়াও") || command.contains("আরও জোরে") -> {
                adjustVolume(AudioManager.ADJUST_RAISE); speakSiri("ভলিউম বাড়িয়ে দিলাম")
            }
            command.contains("volume down") || command.contains("ভলিউম কমাও") || command.contains("শব্দ কমাও") -> {
                adjustVolume(AudioManager.ADJUST_LOWER); speakSiri("ভলিউম কমিয়ে দিলাম")
            }
            command.contains("mute") || command.contains("মিউট") || command.contains("নিরব করো") -> {
            ...
            command.contains("তারিখ") || command.contains("date") || command.contains("today") || command.contains("আজকের তারিখ") -> {
                speakSiri("আজ " + SimpleDateFormat("EEEE, dd MMMM yyyy", Locale("bn", "BD")).format(Date()))
            }
            command.contains("ব্যাটারি") || command.contains("battery") || command.contains("চার্জ কত") -> {
                val level = (getSystemService(BATTERY_SERVICE) as BatteryManager).getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY)
                speakSiri(if (level >= 80) "ব্যাটারি $level শতাংশ। চার্জ ভালো আছে।" else if (level >= 30) "ব্যাটারি $level শতাংশ।" else "ব্যাটারি $level শতাংশ। চার্জ দেওয়ার সময় হয়েছে।")
            }
            ...
            command.contains("call") || command.contains("কল") || command.contains("ফোন") || command.contains("phone") -> {
                speakSiri("ফোন ডায়ালার খুলছি")
                startActivity(Intent(Intent.ACTION_DIAL).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
            }
            command.contains("whatsapp") || command.contains("হোয়াটসঅ্যাপ") || command.contains("হোয়াটসঅ্যাপ") -> openAppAny(listOf("com.whatsapp", "com.whatsapp.w4b"), "WhatsApp")
            command.contains("messenger") || command.contains("মেসেঞ্জার") -> openAppAny(listOf("com.facebook.orca"), "Messenger")
            command.contains("facebook") || command.contains("ফেসবুক") -> openAppAny(listOf("com.facebook.katana", "com.facebook.lite"), "Facebook")
            command.contains("instagram") || command.contains("ইনস্টাগ্রাম") -> openAppAny(listOf("com.instagram.android"), "Instagram")
            command.contains("tiktok") || command.contains("টিকটক") -> openAppAny(listOf("com.zhiliaoapp.musically", "com.ss.android.ugc.trill"), "TikTok")
            command.contains("free fire") || command.contains("ফ্রি ফায়ার") || command.contains("ফ্রি ফায়ার") -> openAppAny(listOf("com.dts.freefireth", "com.dts.freefiremax"), "Free Fire")
            command.contains("chrome") || command.contains("ক্রোম") -> openAppAny(listOf("com.android.chrome"), "Chrome")
            command.contains("gallery") || command.contains("গ্যালারি") || command.contains("ফটো") -> openGallery()
            command.contains("youtube") || command.contains("ইউটিউব") -> {
                val search = extractAfter(command, listOf("youtube", "ইউটিউব"))
                if (search.isBlank() || search in listOf("খুলো", "খোল", "open")) openAppAny(listOf("com.google.android.youtube"), "YouTube", "https://www.youtube.com")
            ...
            command.contains("google") || command.contains("গুগল") -> {
                ...
                if (search.isBlank() || search in listOf("খুলো", "খোল", "open")) openAppAny(listOf("com.google.android.googlequicksearchbox"), "Google", "https://www.google.com")
                else openUrl("https://www.google.com/search?q=" + Uri.encode(search), "গুগলে $search খুঁজে দিচ্ছি")
            }
            command.startsWith("search ") || command.startsWith("খুঁজে ") || command.startsWith("সার্চ ") -> {
            ...
            command.contains("হ্যালো") || command.contains("hello") || command.contains("hi") || command.contains("হাই") ->
                speakSiri(listOf("হ্যালো! আমি JARVIS। কীভাবে সাহায্য করতে পারি?", "হ্যাঁ, বলুন। আমি শুনছি।").random())
            command.contains("তোমার নাম") || command.contains("নাম কি") || command.contains("your name") || command.contains("কে তুমি") ->
                speakSiri("আমার নাম JARVIS। আমি আপনার ব্যক্তিগত ভয়েস অ্যাসিস্ট্যান্ট।")
            command.contains("কেমন আছ") || command.contains("how are you") -> speakSiri("আমি ভালো আছি, ধন্যবাদ!")
            command.contains("ধন্যবাদ") || command.contains("thank you") || command.contains("thanks") ->
                speakSiri(listOf("আপনাকেও ধন্যবাদ", "স্বাগতম").random())
            command.contains("joke") || command.contains("জোক") || command.contains("মজার") ->
                speakSiri(listOf("কম্পিউটার কেন ঠান্ডা থাকে? কারণ সেদের অনেক উইন্ডোজ খোলা থাকে!", "প্রোগ্রামার কেন রাতে ঘুমায় না? কারণ বাগে ভয় পায়।").random())
            command.contains("সাহায্য") || command.contains("help") || command.contains("কী কী করতে পার") ->
                speakSiri("আমি ফোনের যেকোনো অ্যাপ নাম ধরে খুলতে পারি। বন্ধ করতে, হোমে যেতে, ভলিউম নিয়ন্ত্রণ করতে পারি। গুগল সার্চ শুধু তখনই যখন আপনি স্পষ্ট করে বলবেন।")
            else -> {
                if (!openAppByName(command)) speakSiri("দুঃখিত, এটা বুঝতে পারিনি। অন্যভাবে বলবেন?")
            }
        }
    ...
    private fun adjustVolume(direction: Int) {
        (getSystemService(Context.AUDIO_SERVICE) as AudioManager).adjustStreamVolume(AudioManager.STREAM_MUSIC, direction, AudioManager.FLAG_SHOW_UI)
    }
    ...
    override fun onInit(status: Int) {
        ...
        val keys = listOf("female", "woman", "girl", "fem")
        val femaleBn = voices.firstOrNull { it.locale.language == "bn" && keys.any { k -> it.name.contains(k, true) } }
        val femaleAny = voices.firstOrNull { keys.any { k -> it.name.contains(k, true) } }
        try { tts.voice = femaleBn ?: femaleAny ?: tts.voice } catch (_: Exception) {}
        try { tts.setPitch(1.12f); tts.setSpeechRate(0.94f) } catch (_: Exception) {}
